                ...
                <tr>
                    ...
                    <td class="border p-2">
                        <div class="flex gap-2">
                            @if($p->status == 'pending')
                                <form action="{{ route('admin-unitkerja.permohonan.approve', $p->id) }}" method="POST" class="form-approve">
                                    @csrf
                                  
                                    <button type="submit" class="bg-green-500 text-white px-3 py-1 rounded">Setujui</button>
                                </form>
                                <form action="{{ route('admin-unitkerja.permohonan.reject', $p->id) }}" method="POST" class="form-reject">
                                    @csrf
