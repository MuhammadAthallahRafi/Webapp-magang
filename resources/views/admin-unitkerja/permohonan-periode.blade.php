                    ...
                    <tr class="border-b hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex gap-2">
                                <!-- Tombol Lihat Detail -->
                                <a href="{{ route('admin-unitkerja.permohonan.lihat', $p->id) }}" 
                                   class="bg-blue-500 text-white px-3 py-1 rounded text-xs hover:bg-blue-600 transition-colors">
                                    👁️ Detail
                                </a>
                                
                                @if($p->status == 'pending')
                                    <form action="{{ route('admin-unitkerja.permohonan.approve', $p->id) }}" method="POST" class="inline form-approve">
                                        @csrf
                                        <button type="submit" class="bg-green-500 text-white px-3 py-1 rounded text-xs hover:bg-green-600 transition-colors">
                                            ✅ Setujui
                                        </button>
                                    </form>
                                    <form action="{{ route('admin-unitkerja.permohonan.reject', $p->id) }}" method="POST" class="inline form-reject">
                                        @csrf
                                        <button type="submit" class="bg-red-500 text-white px-3 py-1 rounded text-xs hover:bg-red-600 transition-colors">
                                            ❌ Tolak
                                        </button>
                                    </form>
                                @else
