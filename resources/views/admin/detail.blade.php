...
<div class="bg-white border border-gray-200 rounded-lg p-6 mb-6">
    ...
    {{-- ✅ DATA PERMOHONAN YANG SEDANG DILIHAT --}}
    <h2 class="text-xl font-semibold mb-4">Detail Permohonan</h2>
    <div class="bg-white rounded-lg shadow-sm border p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
                ...
                <p class="capitalize">
                    @switch($permohonan->jenis_permohonan)
                        ...
                        @default {{ $permohonan->jenis_permohonan }}
                    @endswitch
                </p>
            </div>
            <div>
                ...
                <p>
                    <span class="px-2 py-1 rounded text-xs font-medium
                        {{ $permohonan->status == 'disetujui' ? 'bg-green-100 text-green-800' : 
                           ($permohonan->status == 'ditolak' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800') }}">
            ...
            <div>
                ...
                <p>{{ $permohonan->tanggal_mulai ? \Carbon\Carbon::parse($permohonan->tanggal_mulai)->format('d/m/Y') : '-' }}</p>
            </div>
            <div>
                <strong>Tanggal Selesai:</strong>
                <p>{{ $permohonan->tanggal_selesai ? \Carbon\Carbon::parse($permohonan->tanggal_selesai)->format('d/m/Y') : '-' }}</p>
            </div>
            <div>
                <strong>Tanggal Pengajuan:</strong>
                <p>{{ $permohonan->tanggal_pengajuan ? \Carbon\Carbon::parse($permohonan->tanggal_pengajuan)->format('d/m/Y') : '-' }}</p>
            </div>
            <div>
    ...
    <div x-data="{ openMulai: false, openSelesai: false }" x-cloak class="flex flex-col sm:flex-row gap-4 mt-4">
        ...
        >
            <div class="bg-white p-5 rounded-xl shadow-lg w-96 relative">
                <form method="POST" action="{{ route('admin.permohonan-periode.updateTanggalMulai', $permohonan->id) }}">
                    @csrf
                    @method('PUT')
        ...
        >
            <div class="bg-white p-5 rounded-xl shadow-lg w-96 relative">
                <form method="POST" action="{{ route('admin.permohonan-periode.updateTanggalSelesai', $permohonan->id) }}">
                    @csrf
                    @method('PUT')
    ...
    @if($permohonan->status === 'pending')
        <div class="flex gap-3 mb-6 mt-6">
            <form action="{{ route('admin.permohonan-periode.approve', $permohonan->id) }}" method="POST">
                @csrf
            ...
            <form action="{{ route('admin.permohonan-periode.reject', $permohonan->id) }}" method="POST">
                @csrf
